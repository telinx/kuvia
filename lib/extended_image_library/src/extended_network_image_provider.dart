...
//        }
  ...
  Future<ui.Codec> _loadAsync(ExtendedNetworkImageProvider key) async {
    ...
    if (cache) {
      try {
        ...
        var data = await DefaultCacheManager().getSingleFile(key.url);
        if (data != null) {
          reuslt = await instantiateImageCodec(data.readAsBytesSync());
        }
      } catch (e) {
        print(e);
      }
  ...
  Future<Uint8List> _loadCache1(ExtendedNetworkImageProvider key, String md5Key) async {
  ...
  Future<Uint8List> _loadCache(ExtendedNetworkImageProvider key, String md5Key) async {
    return DefaultCacheManager().getSingleFile(key.url).then((File file){
      // print('_loadCache----file->>>>>${file.path}');
      if(null != file){
